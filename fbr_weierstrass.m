% -------------------------------------------------------------------
% Arquivo: fbr_weierstrass.m
% Descricao: Interpolacao da funcao de Weierstrass com FBRs.
% -------------------------------------------------------------------
clear; clc; close all;
...
function y = weierstrass(x, a, b)
    y = zeros(size(x));
    for n = 0:100 % Soma ate n=100 e suficiente para convergencia
...
%% --- Pontos para plotagem (malha fina) ---
...
fprintf('--- Analise de Condicionamento ---\n');
for k = 1:length(fbrs)
    rbf = fbrs{k};
